...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if let _ = defaults.objectForKey("token") as? String {
        } else {
        ...
        Alamofire.request(.POST, "http://lyceum.styleru.net/app/api/Autologin", parameters: ["id": (defaults.objectForKey("id") as? Int)!, "token": (defaults.objectForKey("token") as? String)!]) .responseJSON { response in
            if let JSON = response.result.value {
                let code = JSON["error_code"]
                if code as! String != "00" {
                    splitViewController.presentViewController(UIStoryboard(name: "Main", bundle: nil).instantiateViewControllerWithIdentifier("loginController"), animated: true, completion: nil)
                }
            }
        }
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("LYCAPP.sqlite")
